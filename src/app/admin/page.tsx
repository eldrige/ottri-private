...
export default function AdminDashboardPage() {
  ...
  const alerts: { type: 'info' | 'warning' | 'error'; content: React.ReactNode }[] = [
    {
      type: 'info',
      content: (
        <>
    ...
    },
    {
      ...
      content: (
        <>
          Booking <span className='font-bold'>002</span> Status Updated to <span className='font-bold'>“Complete”</span>
        </>
      )
    },
    {
      ...
      content: (
        <>
          Booking <span className='font-bold'>008</span> Status Updated to <span className='font-bold'>“In-Progress”</span>
        </>
      )
    },
    {
      ...
      content: (
        <>
          Booking <span className='font-bold'>012</span> has been <span className='font-bold'>“Cancelled”</span>
        </>
      )
    }
  ];
      ...
      <div className='mt-6 flex *:flex-1 gap-4 flex-col lg:flex-row'>
        <div className='lg:border border-black/10 lg:p-6 rounded-lg'>
          ...
          <div className='mt-6 space-y-2.5'>
            {scheduleItems.map((item, index) => (
              <div key={index} className='border border-black/10 rounded-lg py-2 px-4 flex flex-col lg:flex-row lg:items-center gap-3 justify-between'>
                <div className='space-y-1'>
                  <p>
                    <span className='font-medium'>{item.customerName}</span>
                    <span className={`text-sm py-1 px-4 ml-3 rounded-lg ${item.statusColor}`}>{item.status}</span>
                  </p>
                  ...
                  <p className='text-xs text-secondary-700/70'>Cleaner:<span className="ml-2">{item.cleaner}</span></p>
                </div>
                <div className='flex gap-3 *:flex-1 lg:*:flex-auto'>
                  {item.buttonText && (
        ...
        <div className='lg:border border-black/10 lg:p-6 rounded-lg'>
          ...
          <div className="mt-6 space-y-4">
            {alerts.map((a, idx) => (
              <div key={idx} className={`p-4 flex flex-col lg:flex-row gap-2 lg:items-center justify-between rounded-lg border ${alertStyles[a.type]}`}>
                <p>{a.content}</p>
                <button className="py-2 px-4 text-xs rounded-lg border border-black/10 bg-white flex-1 lg:flex-auto">View</button>
              </div>
            ))}
          </div>
        </div>
