...
export default function LandingSection4() {
  return (
    <div className="flex w-full text-center flex-col gap-16 py-24">
      ...
      <ReviewSlider />
      <ul className="pt-8  w-full grid grid-cols-1 sm:grid-cols-2 text-lg lg:grid-cols-4 text-surface-500 *:items-center *:flex md:*:justify-center *:gap-2 gap-y-7">
        <li>
          <CheckIcon />
          <p>Insured & Bonded</p>
        </li>
          ...
          {reviews.map((_, i) => (
            <button
              ...
              className={`size-3 rounded-full ${
                current === i ? "bg-primary-700" : "bg-surface-300"
              }`}
            />
          ))}
        </div>
